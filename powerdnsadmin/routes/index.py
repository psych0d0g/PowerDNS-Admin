...
def login():
    ...
    if "azure_token" in session:
        ...
        azure_info = azure.post(
            "me/getMemberGroups", json={"securityEnabledOnly": False}
        ).text
        ...
        if not user:
            ...
            if not result["status"]:
                ...
                return render_template(
                    ...
                    error=("User " + azure_username + " cannot be created."),
                )
        ...
        if Setting().get("azure_group_accounts_enabled") and mygroups:
            ...
            mygroups = get_azure_groups(
                "me/memberOf/microsoft.graph.group?$count=false&$securityEnabled=true&$select={}".format(
                    select_values
                )
            )
                ...
                if name_value in azure_group:
                    ...
                    if description_value in azure_group:
                        ...
                        if description_pattern != "":
                            current_app.logger.info(
                                "Matching group description {} against regex {}".format(
                    ...
                    if pattern != "":
                        current_app.logger.info(
                            "Matching group name {} against regex {}".format(
                                group_name, pattern
                            )
                        )
                    ...
                    if account_id:
                        ...
                        current_app.logger.info(
                            "Group: {} Users: {}".format(group_name, account_users)
                        )
                        if user.id in account_users:
                            current_app.logger.info(
                                "User id {} is already in account {}".format(
                                    user.id, group_name
                                )
                            )
                        else:
    ...
    if request.method == "GET":
        ...
        try:
            ...
            current_app.logger.error("Cannot authenticate user. Error: {}".format(e))
...
def signin_history(username, authenticator, success):
    ...
    # Write history
    History(
        msg="User {} authentication {}".format(username, str_success),
        detail=json.dumps(
            {
                "username": username,
...
def logout():
    if (
        current_app.config.get("SAML_ENABLED")
        and "samlSessionIndex" in session
        and current_app.config.get("SAML_LOGOUT")
    ):
    ...
    if current_app.config.get("REMOTE_USER_ENABLED") and remote_user_logout_url:
        current_app.logger.debug(
            'Redirecting remote user "{0}" to logout URL {1}'.format(
                current_user.username, remote_user_logout_url
            )
        )
...
def password_policy_check(user, password):
    ...
    if Setting().get("pwd_enforce_characters") or Setting().get(
    ...
    if Setting().get("signup_enabled"):
        ...
        if request.method == "GET":
            ...
            if not password_policy_pass:
                return render_template(
                    "register.html",
                    error_messages=password_policy,
                    captcha_enable=CAPTCHA_ENABLE,
                )
            ...
            try:
                ...
                if result and result["status"]:
                    ...
                    if Setting().get("otp_force") and Setting().get(
                        "otp_field_enabled"
                    ):
                        user.update_profile(enable_otp=True)
                ...
                return render_template(
                    "register.html", error=e, captcha_enable=CAPTCHA_ENABLE
                )
        else:
...
def welcome():
    ...
    if request.method == "GET":
        return render_template(
            "register_otp.html", qrcode_image=encoded_img_data.decode(), user=user
        )
    elif request.method == "POST":
        otp_token = request.form.get("otptoken", "")
        if otp_token and otp_token.isdigit():
            ...
            if not good_token:
                return render_template(
                    "register_otp.html",
                    qrcode_image=encoded_img_data.decode(),
                    user=user,
                    error="Invalid token",
                )
        else:
...
def confirm_email(token):
    ...
    if user.confirmed:
        # Already confirmed
        current_app.logger.info("User email {} already confirmed".format(email))
        return render_template("email_confirmation.html", status=2)
    else:
        # Confirm email is valid
        user.update_confirmed(confirmed=1)
        current_app.logger.info("User email {} confirmed successfully".format(email))
...
def dyndns_update():
    ...
    dualstack_update = request.args.get("dualstack_update") == "1"
    ...
    all_ips = myip_addr or remote_addr
    records_by_type = {"A": [], "AAAA": []}

    for ip in all_ips:
        if isinstance(ip, ipaddress.IPv4Address):
            records_by_type["A"].append(str(ip))
        else:
            records_by_type["AAAA"].append(str(ip))

    response = "nochg"

    for rtype, ip_list in records_by_type.items():
        if not ip_list and not dualstack_update:
            continue

        r = Record(name=hostname, type=rtype)

        if r.exists(domain.name) and r.is_allowed_edit():
            existing_ips = set(r.data.split(",") if "," in r.data else [r.data])
            if set(ip_list) == existing_ips:
                continue  # No changes
            ...
            rrset_data = [
                {
                    ...
                    "ttl": 3600,
                    "type": rtype,
                    "records": [{"content": ip, "disabled": False} for ip in ip_list],
                    "comments": [],
                }
            ]
            ...
            if result["status"] == "ok":
                History(
                    msg=f"DynDNS update: updated {hostname} ({rtype}) successfully",
                    detail=json.dumps(
                        {
                            "domain": domain.name,
                            "record": hostname,
            ...
            setting = DomainSetting.query.filter_by(
                domain=domain, setting="create_via_dyndns"
            ).first()
            if setting and strtobool(setting.value):
                rrset_data = [
                    {
                        "changetype": "REPLACE",
                        "name": hostname + ".",
                        ...
                        "type": rtype,
                        "records": [
                            {"content": ip, "disabled": False} for ip in ip_list
                        ],
                        "comments": [],
                    }
                ]
                ...
                if result["status"] == "ok":
                    History(
                        msg=f"DynDNS update: created record {hostname} ({rtype}) in zone {domain.name}",
                        detail=json.dumps(
                            {
                                "domain": domain.name,
                                "record": hostname,
            ...
            History(
                msg=f"DynDNS update: attempted update of {hostname} but not allowed or not found",
                created_by=current_user.username,
            ).add()
    ...
    if len(errors) == 0:
        ...
        if current_app.config.get("SAML_ATTRIBUTE_USERNAME", False):
            username = session["samlUserdata"][
                current_app.config["SAML_ATTRIBUTE_USERNAME"]
            ][0].lower()
        else:
        ...
        session["user_id"] = user.id
        email_attribute_name = current_app.config.get("SAML_ATTRIBUTE_EMAIL", "email")
        ...
        name_attribute_name = current_app.config.get("SAML_ATTRIBUTE_NAME", None)
        account_attribute_name = current_app.config.get("SAML_ATTRIBUTE_ACCOUNT", None)
        admin_attribute_name = current_app.config.get("SAML_ATTRIBUTE_ADMIN", None)
        group_attribute_name = current_app.config.get("SAML_ATTRIBUTE_GROUP", None)
        admin_group_name = current_app.config.get("SAML_GROUP_ADMIN_NAME", None)
        operator_group_name = current_app.config.get("SAML_GROUP_OPERATOR_NAME", None)
...
def create_group_to_account_mapping():
    ...
    if (
        group_to_account_mapping_string
...
def handle_account(account_name, account_description=""):
    ...
    if not account:
        account = Account(
            name=clean_name, description=account_description, contact="", mail=""
        )
        account.create_account()
        history = History(
...
def swagger_spec():
    try:
        ...
        current_app.logger.error("Cannot view swagger spec. Error: {0}".format(e))
        current_app.logger.debug(traceback.format_exc())
        abort(500)
